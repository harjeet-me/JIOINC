...
export class InsuranceService {
  ...
  protected convertDateFromClient(insurance: IInsurance): IInsurance {
    const copy: IInsurance = Object.assign({}, insurance, {
      issueDate: insurance.issueDate != null && insurance.issueDate.isValid() ? insurance.issueDate.format(DATE_FORMAT) : null,
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((insurance: IInsurance) => {
        insurance.issueDate = insurance.issueDate != null ? moment(insurance.issueDate) : null;
