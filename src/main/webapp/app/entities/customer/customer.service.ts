...
export class CustomerService {
  ...
  protected convertDateFromClient(customer: ICustomer): ICustomer {
    const copy: ICustomer = Object.assign({}, customer, {
      customerSince: customer.customerSince != null && customer.customerSince.isValid() ? customer.customerSince.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.customerSince = res.body.customerSince != null ? moment(res.body.customerSince) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((customer: ICustomer) => {
        customer.customerSince = customer.customerSince != null ? moment(customer.customerSince) : null;
      });
    }
    return res;
  }
